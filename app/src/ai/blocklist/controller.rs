        ...
        ctx.subscribe_to_model(&action_model, move |me, event, ctx| {
            ...
            if !should_trigger_follow_up_request {
                    ...
                    .has_active_stream_for_conversation(*conversation_id, ctx)
                {
                    ...
                    // QUALITY-780 §10: the previous §8.2 ordering guard
                    // (skip the `Success` transition when the conversation
                    // is `WaitingForEvents`) is no longer needed. With
                    // `wait_for_events` running through the action_model
                    // as a first-class async action, the wait action
                    // stays in `running_actions` for the duration of the
                    // yield — so this `FinishedAction` subscriber only
                    // runs after the wait completes and the conversation
                    // is already on its way back to `InProgress` via the
                    // executor's `complete_wait_action` path.
