...
// QUALITY-780 §10: the waiting-watchdog helpers (`find_unresolved_wait_for_events_call`,
// `watchdog_timeout_for_call`, `UnresolvedWaitForEventsCall`,
// `DEFAULT_ORCHESTRATED_IDLE_TIMEOUT_SECONDS`) and their tests have moved
// to `app/src/ai/blocklist/action_model/execute/wait_for_events.rs`. The
// driver no longer schedules the watchdog itself; the
// `WaitForEventsExecutor` owns that lifecycle.
